...
def main():
    parser = argparse.ArgumentParser(description="Minimal resource monitoring tool for Operating Systems. Sends a mail with the current state of the resources of the servers specified in pmp.conf.")

    parser.add_argument('-c', '--check-config', action="store_true", help="Performs a verification of the configuration file and exists")
    parser.add_argument('config_file')

    args = parser.parse_args()

    if not os.path.isfile(args.config_file) and not os.path.exists(args.config_file):
        parser.error("Configuration file isn't valid or doesn't exists.\n")

    servidores = check_config_file(args.config_file,args.check_config)
    recursos_serv = {}
